...
p_baseName = 'Porph_';
f_baseName = 'Porph_';      %Higher laser power data
suffix = '.txt';
suffix2 = '2.txt';
...
%     plot(f(plotInd),unwrap(angle(fSpectrum(plotInd)))*180/pi,'*')
    ...
    fluor(i).f = f(ind);        %Fluor represents higher power data
%    fluor(i).f2 = f(ind2f);
...
%     porph(i).phase2f = temp*180/pi;
    ...
    lowPwrPhase = porph(i).phase - 190;
    highPwrPhase = fluor(i).phase - 190;
...
plot(freqs,lowPwrPhase,'r*',freqs, highPwrPhase, 'b*',freqs,theorPhase1f,'r')
title('1F phase shift')
xlabel('Modulation freq (Hz)')
ylabel('Phase shift (degrees)')
legend('Low Power','High Power','Theoretical')
